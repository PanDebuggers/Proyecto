...
def mostrar_tomas_dia(frame, id_cuidador):
    ...
    if not tomas:
        tk.Label(
            frame, text="No hay tomas programadas para esta semana.", fg="gray"
        ).pack(pady=10)
    ...
    def marcar_toma(estado):
        ...
        if not seleccion:
            messagebox.showwarning(
                "Selecciona una toma", "Por favor selecciona una toma de la lista."
            )
            return
        item = seleccion[0]
    ...
    btn_tomada = ctk.CTkButton(
        btn_frame,
        text="Marcar como Tomada",
        command=lambda: marcar_toma("tomada"),
        fg_color="#90EE90",
    )
    btn_tomada.pack(side="left", padx=10)
    btn_omitida = ctk.CTkButton(
        btn_frame,
        text="Marcar como Omitida",
        command=lambda: marcar_toma("omitida"),
        fg_color="#FF6347",
    )
