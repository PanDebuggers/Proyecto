...
def mostrar_tratamientos(frame_dinamico):
    ...
    ctk.CTkOptionMenu(
        filtro_frame, values=["Todos"] + pacientes_nombres, variable=filtro_paciente
    ).pack(side="left", padx=5)
    ctk.CTkLabel(filtro_frame, text="Estado:").pack(side="left", padx=5)
    estados = ["activo", "suspendido", "finalizado", "pendiente"]
    ctk.CTkOptionMenu(
        filtro_frame, values=["Todos"] + estados, variable=filtro_estado
    ).pack(side="left", padx=5)
    ctk.CTkLabel(filtro_frame, text="Buscar tratamiento:").pack(side="left", padx=5)
    tk.Entry(filtro_frame, textvariable=filtro_tratamiento, width=20).pack(
        side="left", padx=5
    )
    ...
    ctk.CTkButton(
        ...
        command=lambda: mostrar_formulario_tratamiento(
            main_frame, tabla, pacientes_nombres, estados, cargar_tabla
        ),
    ).pack(side="left", padx=5)
    ctk.CTkButton(
        btns_frame,
        ...
        fg_color="#d9534f",
        hover_color="#c9302c",
        command=lambda: eliminar_tratamiento(tabla, cargar_tabla),
    ).pack(side="left", padx=5)
    ctk.CTkButton(
        btns_frame,
        text="Asignar medicamento",
        command=lambda: mostrar_formulario_medicamento(main_frame, tabla, cargar_tabla),
    ).pack(side="left", padx=5)
    ctk.CTkButton(btns_frame, text="Actualizar", command=lambda: cargar_tabla()).pack(
        side="right", padx=5
    )
    ...
    def cargar_tabla():
        ...
        try:
            ...
            for t in tratamientos:
                p = next(
                    (p for p in pacientes if p["id_paciente"] == t["id_paciente"]), None
                )
                if not p:
                    continue
                ...
                if (
                    paciente_filtro
                    and paciente_filtro != "Todos"
                    and nombre_paciente != paciente_filtro
                ):
                    continue
                if (
                    estado_filtro
                    and estado_filtro != "Todos"
                    and t["estado"] != estado_filtro
                ):
                    continue
...
def mostrar_formulario_tratamiento(
    parent_frame, tabla, pacientes_nombres, estados, cargar_tabla_callback
):
    global open_dialogs
    ...
    row = 0
    # Paciente
    ctk.CTkLabel(frame, text="Paciente:").grid(
        row=row, column=0, sticky="w", pady=5, padx=5
    )
    if pacientes_nombres:
        ctk.CTkOptionMenu(frame, values=pacientes_nombres, variable=paciente_var).grid(
            row=row, column=1, sticky="ew", pady=5, padx=5
        )
    else:
        ctk.CTkLabel(frame, text="No hay pacientes", fg_color="red").grid(
            row=row, column=1, sticky="ew", pady=5, padx=5
        )
    row += 1

    # Tratamiento
    ctk.CTkLabel(frame, text="Tratamiento:").grid(
        row=row, column=0, sticky="w", pady=5, padx=5
    )
    ctk.CTkEntry(frame, textvariable=tratamiento_var).grid(
        row=row, column=1, sticky="ew", pady=5, padx=5
    )
    row += 1

    # Objetivo
    ctk.CTkLabel(frame, text="Objetivo:").grid(
        row=row, column=0, sticky="w", pady=5, padx=5
    )
    ...
    # Fecha inicio
    ctk.CTkLabel(frame, text="Fecha inicio (YYYY-MM-DD):").grid(
        row=row, column=0, sticky="w", pady=5, padx=5
    )
    ctk.CTkEntry(
        frame, textvariable=fecha_inicio_var, placeholder_text="2024-01-15"
    ).grid(row=row, column=1, sticky="ew", pady=5, padx=5)
    row += 1

    # Fecha fin
    ctk.CTkLabel(frame, text="Fecha fin (YYYY-MM-DD):").grid(
        row=row, column=0, sticky="w", pady=5, padx=5
    )
    ctk.CTkEntry(frame, textvariable=fecha_fin_var, placeholder_text="2024-02-15").grid(
        row=row, column=1, sticky="ew", pady=5, padx=5
    )
    row += 1

    # Estado
    ctk.CTkLabel(frame, text="Estado:").grid(
        row=row, column=0, sticky="w", pady=5, padx=5
    )
    ctk.CTkOptionMenu(frame, values=estados, variable=estado_var).grid(
        row=row, column=1, sticky="ew", pady=5, padx=5
    )
    row += 1
    ...
    ctk.CTkButton(
        btn_frame,
        text="Cancelar",
        command=lambda: cerrar_ventana_simple(win),
        fg_color="gray",
        hover_color="#666666",
    ).grid(row=0, column=0, sticky="ew", padx=(0, 5))
    ctk.CTkButton(btn_frame, text="Guardar", command=guardar).grid(
...
def mostrar_formulario_medicamento(parent_frame, tabla, cargar_tabla_callback):
    ...
    if not sel:
        messagebox.showwarning(
            "Selecciona", "Selecciona un tratamiento para asignar medicamento."
        )
        return
    ...
    row = 0
    # Medicamento
    ctk.CTkLabel(frame, text="Medicamento:").grid(
        row=row, column=0, sticky="w", pady=5, padx=5
    )
    if medicamentos_nombres:
        medicamento_var.set(medicamentos_nombres[0])
        ctk.CTkOptionMenu(
            frame, values=medicamentos_nombres, variable=medicamento_var
        ).grid(row=row, column=1, sticky="ew", pady=5, padx=5)
    else:
        ctk.CTkLabel(frame, text="No hay medicamentos", fg_color="red").grid(
            row=row, column=1, sticky="ew", pady=5, padx=5
        )
    row += 1

    # Dosis
    ctk.CTkLabel(frame, text="Dosis:").grid(
        row=row, column=0, sticky="w", pady=5, padx=5
    )
    ctk.CTkEntry(frame, textvariable=dosis_var, placeholder_text="ej: 500mg").grid(
        row=row, column=1, sticky="ew", pady=5, padx=5
    )
    row += 1

    # Frecuencia
    ctk.CTkLabel(frame, text="Frecuencia:").grid(
        row=row, column=0, sticky="w", pady=5, padx=5
    )
    ctk.CTkOptionMenu(frame, values=frecuencia_opciones, variable=frecuencia_var).grid(
        row=row, column=1, sticky="ew", pady=5, padx=5
    )
    row += 1

    # Vía de administración
    ctk.CTkLabel(frame, text="Vía administración:").grid(
        row=row, column=0, sticky="w", pady=5, padx=5
    )
    ctk.CTkOptionMenu(frame, values=via_opciones, variable=via_var).grid(
        row=row, column=1, sticky="ew", pady=5, padx=5
    )
    row += 1
    ...
    ctk.CTkOptionMenu(frame, values=estado_opciones, variable=estado_var).grid(
        row=row, column=1, sticky="ew", pady=5, padx=5
    )
    row += 1
    ...
    def guardar():
        try:
            ...
            asignacion_id = logic_tratamientos.asignar_medicamento_a_tratamiento(
                **datos
            )
            logic_tratamientos.generar_tomas_tratamiento(asignacion_id)
            messagebox.showinfo(
                "Éxito", "Medicamento asignado y tomas generadas correctamente."
            )
            cerrar_ventana_simple(win)
        except Exception as e:
