...
def iniciar_aplicacion(email, root):
    ...
    if os.path.exists(IMG_PATH):
        ...
        tk.Label(
            frame_dinamico,
            text="Bienvenido a MediCareDesk",
            bg="#f0f0f0",
            font=("Arial", 16),
        ).pack(expand=True)
    ...
    menu_lateral.btn_pacientes.configure(
        command=lambda: mostrar_pacientes(frame_dinamico, id_cuidador)
    )
    menu_lateral.btn_medicamentos.configure(
        command=lambda: mostrar_medicamentos(frame_dinamico)
    )
    menu_lateral.btn_tratamientos.configure(
        command=lambda: mostrar_tratamientos(frame_dinamico)
    )
    menu_lateral.btn_tomas_dia.configure(
        command=lambda: __mostrar_tomas_dia(frame_dinamico, id_cuidador)
    )
    menu_lateral.btn_historial_tomas.configure(
        command=lambda: __mostrar_historial_tomas(frame_dinamico, id_cuidador)
    )
    menu_lateral.btn_alertas.configure(
        command=lambda: __mostrar_alertas(frame_dinamico, id_cuidador)
    )
