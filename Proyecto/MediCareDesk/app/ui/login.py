...
class LoginView(tk.Frame):
    ...
    def crear_interfaz(self):
        ...
        self.password_entry = tk.Entry(
            container, width=35, show="*", font=("Arial", 13)
        )

        for label, entry in [
            ("Correo electrónico", self.email_entry),
            ("Contraseña", self.password_entry),
        ]:
            tk.Label(container, text=label, font=("Arial", 12), bg="#f0f0f0").pack(
                anchor="w"
            )
            entry.pack(pady=5)

        ctk.CTkButton(
            container, text="Iniciar sesión", command=self.iniciar_sesion, width=200
        ).pack(pady=(20, 10))
        ctk.CTkButton(
            container,
            text="Registrarse",
