...
-- Vista de Tomas Programadas por Medicamento
CREATE OR REPLACE VIEW `Vista_Tomas_Programadas` AS
SELECT 
    p.nombre AS paciente,
    m.nombre AS medicamento,
    tm.dosis,
...
-- Vista de Tomas Verificadas por el Cuidador
CREATE OR REPLACE VIEW `Vista_Tomas_Verificadas` AS
SELECT 
    p.nombre AS paciente,
    m.nombre AS medicamento,
    t.hora_programada,
...
-- Vista de Búsqueda Eficiente de Pacientes o Medicamentos
CREATE OR REPLACE VIEW `Vista_Busqueda_Pacientes_Medicamentos` AS
SELECT
    p.id_paciente,
    p.nombre AS paciente,
    m.id_medicamento,
...
JOIN Paciente p ON tr.id_paciente = p.id_paciente
JOIN Medicamento m ON tm.id_medicamento = m.id_medicamento
WHERE t.fecha = CURDATE()  -- Solo las tomas del día de hoy
ORDER BY t.hora_programada;


-- Procedimientos almacenados

DELIMITER //  -- autentificacion
...
DELIMITER // -- GenerarTomasTratamiento`
...
BEGIN
    ...
    -- Obtener frecuencia y hora preferida del tratamiento
    ...
    -- Si no se encuentra la frecuencia, lanzar error
    IF v_frecuencia IS NULL THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'Frecuencia no encontrada para el tratamiento';
    END IF;

    -- Configurar el número de tomas por día según la frecuencia
    CASE v_frecuencia
        ...
        WHEN 'personalizada' THEN 
            SET v_tomas_por_dia = 1; -- Aquí debes agregar lógica si la frecuencia personalizada es diferente
        ELSE
            SET v_tomas_por_dia = 1;
    END CASE;

    -- Generar las tomas para el rango de fechas proporcionado
    ...
    WHILE v_fecha_actual <= p_fecha_fin DO
        ...
        WHILE v_contador < v_tomas_por_dia DO
            -- Calcular la hora de la toma para la frecuencia establecida
            SET v_hora_toma = ADDTIME(v_hora_preferida, SEC_TO_TIME(v_contador * (86400 / v_tomas_por_dia)));

            -- Insertar la toma programada en la base de datos
...
DELIMITER ;
 ...
 -- Procedimiento: AsignarMedicamentoATratamiento
DELIMITER //
-- Eliminar el procedimiento si ya existe
...
-- Procedimiento: MarcarTomaComoVerificada
DELIMITER //
-- Eliminar el procedimiento si ya existe
...
-- Procedimiento: MarcarTomaComoOmitida
DELIMITER //
...
-- Procedimiento: ObtenerHistorialTomas
DELIMITER //
...
-- Procedimiento: GenerarReporteTomasPaciente
DELIMITER //
...
SET SQL_SAFE_UPDATES = 0;   -- permite actualizar, cambiar a 1 para desactivar


-- Insertar un paciente de prueba
INSERT INTO Paciente (nombre, edad, genero, contacto_emergencia, observaciones) 
VALUES ('Juan Pérez', 65, 'M', '3001234567', 'Paciente con hipertensión');

-- Verificar que el paciente se insertó
SELECT * FROM Paciente WHERE nombre = 'Juan Pérez';

-- Actualizar los datos del paciente
UPDATE Paciente SET edad = 66 WHERE nombre = 'Juan Pérez';

-- Asignar un cuidador a un paciente
INSERT INTO Paciente_Cuidador (id_paciente, id_cuidador)
VALUES (1, 1);  -- Asumiendo que ya tienes un paciente y un cuidador insertados

-- Verificar la relación entre paciente y cuidador
SELECT * FROM Paciente_Cuidador WHERE id_paciente = 1;
...
-- Probar la restricción de claves foráneas al eliminar un paciente con un cuidador asignado
DELETE FROM Paciente WHERE id_paciente = 1;  -- Ver si la restricción de integridad referencial funciona
